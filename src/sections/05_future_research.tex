In this manuscript we focus on the idea to combine the Newton's interpolation formula and the hockey-stick family identities
for binomial coefficients to express the sums of powers seamlessly.

This particular idea is great, however it can be generalized even further, so that the main aim becomes to
utilize an interpolation formula for power $n^m$ in terms of an \textit{abstract difference operator} $D(n^m)$ and binomial coefficients $\binom{f(n)}{k}$
such that $n$ indicates the variable of power function.
The difference operator can be arbitrary, for example: forward, backward, or central differences.
For instance, the abstract interpolation formula is
\begin{align*}
...
\end{align*}
Many interpolation approaches involve rising factorials $x_{(n)}$, falling factorials $(x)_n$, or factorials $n!$,
...
In particular, Donald Knuth provides a formula multifold sums of odd powers~\cite{knuth1993johann}
based on the operator of central finite differences of power evaluated in zero, that is
\begin{proposition}[Multifold sums of odd powers]
    ...
    \end{align*}
\end{proposition}
where $T(n,k)$ are thr central factorial numbers of the second
...
Knuth's formula~\eqref{prop:knuth-sums-of-odd-powers} utilizes the operator
