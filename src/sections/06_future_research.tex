...
Newton's interpolation formula for forward
differences~\eqref{prop:newtons-interpolation-formula-for-powers}
and hockey-stick family
...
Thus, the main idea is to utilize an interpolation formula for power $n^m$
in terms of \textit{abstract difference operator} $D(n^m)$
...
Bohner and Peterson~\cite{Bohner2001DynamicEO}.
...
\end{align*}
Thus, the formula of sums of powers involves the abstract difference operator
$D$ evaluated at some point $k$ and the hockey-stick
...
falling factorials $\fallingFactorial{x}{n}$, or regular factorials $n!$,
...
odd powers~\cite{knuth1993johann}
based on operator of central finite differences evaluated in zero,
and Newton's interpolation formula for central
differences~\cite{kolosov_2025_18098442}
