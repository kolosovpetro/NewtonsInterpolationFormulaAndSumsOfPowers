...
forward Newton's interpolation formula for powers
...
\begin{align*}
  \begin{cases}
    ...
    f(x)
    &= \sum_{k=0}^{\infty} \frac{\risingFactorial{(x-a)}{k}}{k!} \Delta^k f(a)
    = \sum_{k=0}^{\infty} \binom{x-a}{k} \Delta^k f(a), \\
    f(x)
    &= \sum_{k=0}^{\infty} \frac{\fallingFactorial{x-a}{k}}{k!} \nabla^k f(a)
...
of integers.
In this manuscript, we focus on closed forms of powers sums,
by utilizing forward Newton's formula, and hockey-stick
