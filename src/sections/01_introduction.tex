...
\begin{proposition}[Newton's series for power]
    For non-negative integers $m,n$ and arbitrary integer $t$
...
Thus, for arbitrary integer $t$, the ordinary sum of powers is
\begin{align*}
    \KnuthRFoldSum{1}{n}{m}
...
in the OEIS~\cite{sloane2003line}.
To obtain the formula for double sum of powers, we simply apply summation operator over the ordinary sum of powers again,
thus
\begin{align*}
...
\begin{theorem}[Multifold sums of powers via Newton's series]
    ...
    For non-negative integers $r,n,m$ and arbitrary integer $t$
    \begin{align*}
    ...
    \begin{proof}
        ...
        segmented hockey stick identity~\eqref{prop:segmented-hockey-stick-identity}.
    \end{proof}
\end{theorem}
We may observe that
...
\begin{proposition}[Multifold sums of powers binomial form reindexed]
    ...
    \end{align*}
\end{proposition}
Finite difference of power is closely related to Stirling numbers of the second kind
\begin{lemma}[Finite difference via Stirling numbers]
    \label{prop:finite-differences-via-stirling-numbers}
    For non-negative integers $j,m$ and arbitrary integer $t$
    \begin{align*}
        \Delta^{j} t^{m} = \sum_{k=0}^{m} \binom{t}{k} \stirlingii{m}{j+k} (j+k)!
    \end{align*}
\end{lemma}
which implies the variations of the formulas for sums of powers
\begin{proposition}[Ordinary sums of powers via Stirling numbers]
    ...
    \begin{proof}
        ...
        and finite difference via Stirling numbers of the second kind~\eqref{prop:finite-differences-via-stirling-numbers}.
    \end{proof}
\end{proposition}
In general,
\begin{proposition}[Multifold sums of powers via Stirling numbers]
    ...
    \begin{align*}
        &\KnuthRFoldSum{r}{n}{m} \\
        &= \sum_{j=0}^{m} \sum_{k=0}^{m} \left[ \left( \sum_{s=1}^{r} (-1)^{j+s-1} \binom{j+t-1}{j+s} \KnuthRFoldSum{r-s}{n}{0} \right) + \binom{n-t+r}{j+r} \right] \binom{t}{k} \stirlingii{m}{j+k} (j+k)!
    \end{align*}
    \begin{proof}
...
In addition, we are capable to express multifold sums of powers via Eulerian numbers, by
expressing the forward finite difference via Worpitzky identity~\cite{Worpitzky1883}
...
\begin{proposition}[Multifold sums of powers via Eulerian numbers]
    ...
    \begin{align*}
        &\KnuthRFoldSum{r}{n}{m} \\
        &= \sum_{j=0}^{m} \sum_{k=0}^{m} \left[ \left( \sum_{s=1}^{r} (-1)^{j+s-1} \binom{j+t-1}{j+s} \KnuthRFoldSum{r-s}{n}{0} \right) + \binom{n-t+r}{j+r} \right] \eulerian{m}{k} \binom{t+k}{m-j}
    \end{align*}
    \begin{proof}
        By multifold sums of powers via Newton's series~\eqref{theorem:multifold-sums-of-powers-via-newtons-series}
