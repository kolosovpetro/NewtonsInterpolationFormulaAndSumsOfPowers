...
\begin{proposition}
    ...
    are $k$-degree forward finite differences of $f$ evaluated in point $a$.
\end{proposition}
...
\begin{proposition}[Multifold sums of powers via Eulerian numbers]
    ...
    \begin{mdframed}
        For non-negative integers $r,n,m$ and an arbitrary integer $t$
        \begin{align*}
            \KnuthRFoldSum{r}{n}{m}= \sum_{j=0}^{m} \sum_{k=0}^{m} \left[ \left( \sum_{s=1}^{r} (-1)^{j+s-1} \binom{j+t-1}{j+s} \KnuthRFoldSum{r-s}{n}{0} \right) + \binom{n-t+r}{j+r} \right] \binom{t+k}{m-j} \eulerian{m}{k}
        \end{align*}
    \end{mdframed}
    \begin{proof}
        By multifold sums of powers via Newton's series~\eqref{theorem:multifold-sums-of-powers-via-newtons-series}
